...
from student_management_app.models import CustomUser, Staffs, Courses, \
    Subjects, Students, SessionYearModel, Attendance, AttendanceReport, LeaveReportStaff, StudentResult,Message
...
def staff_add_result_save(request):
    if request.method != "POST":
        ...
        try:
            check_exist = StudentResult.objects.filter(subject_id=subject, student_id=student).exists()
            if check_exist:
                result = StudentResult.objects.get(subject_id=subject, student_id=student)
                result.subject_exam_marks = exam_marks
                result.save()
                messages.success(request, "Result updated successfully!")
                return redirect('staff_add_result')
            else:
                result = StudentResult(student_id=student, subject_id=subject,
...
def create_messagee(request):
    message = Message.objects.all()
    context = {"message": message}
    return render(request, "staff_template/message.html", context)


def create_message_savee(request):
    if request.method != "POST":
