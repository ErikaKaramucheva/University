...
namespace LinqSamples
{
    class Test
    {
        ...
        //Find first product by ProductID, or if such a product does not exists return null
        public Product findProductByID(int productID)
        {
            return productList.Where(x => x.ProductID == productID).FirstOrDefault();
        }
        //Find first product by ProductID, or if such a product does not exists return null
        public Product findFirstProductByID(int productID)
        {
            return productList.Where(x => x.ProductID == productID).First();
        }
        public Product FindElementByIndex(int index)
        {
            return productList.ElementAt(index);
        }
        //Return product by index in the list or if such does not exists return null
        public Product FindElementByIndex2(int index)
        {
            return productList.ElementAtOrDefault(index);
        }
        //Find all products in stock
        public IEnumerable<Product> FindProductsInStocks()
        {
            var prod= productList.Where(x => x.UnitsInStock > 0);
            return prod.ToArray();
        }
